...
try {
  ...
  client.on("messageCreate", async (message) => {
    ...
    if (message.content.startsWith("!export")) {
      ...
      const allMessages = new Collection<string, Message | PartialMessage>();
      ...
      if (split.length == 1 && split[0] == "all") {
        ...
        let messages: Collection<string, Message | PartialMessage> =
          new Collection();
        // while((await channel.messages.fetch()).array().length > 0) {
        // await channel.messages.fetch();
        while (
          (messages = await (<TextChannel>channel).bulkDelete(100, true))
            .size >= 100
        ) {
          messages.forEach((message) => {
      ...
      pyExpressions.forEach(async (res, key) => {
        try {
          ...
          // @<494009206341369857>
          (message.guild.channels.cache.get(botChannelId) as TextChannel).send(
    ...
    res.on("end", async () => {
      ...
      if (data.found > count) {
        ...
        const embed = new EmbedBuilder()
          ...
          .setFooter({
            text: "Automatically detected by a bot. Please report any issues",
          })
          .setURL(data.posts[0].URL);
        (<TextChannel>await client.channels.cache.get(blogId)?.fetch()).send({
          embeds: [embed],
        });
        count = data.found;
        resolve(count);
