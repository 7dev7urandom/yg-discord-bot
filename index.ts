...
import {
    ActionExpression,
    BooleanExpression,
    PythonActionExpression,
    PythonBooleanExpression,
} from "./expression";
...
let responses = ["Stop bullying me"];
...
db.serialize(() => {
    db.all(`SELECT value FROM responses`, [], (err, rows) => {
        ...
        if (responses.length == 0) {
            ...
            db.run(
                `INSERT INTO responses (value) VALUES ${responses
                    .map((_) => "(?)")
                    .join(",")}`,
                responses,
                (err) => {
...
try {
    ...
    client.on("messageCreate", async (message) => {
        if (message.channel.type === "DM") {
            console.log(
                "[DM] " + message.author.username + ": " + message.content
            );
        }
        if (message.channel.id === bibleVerseAdminId) {
            const collected = await message.awaitReactions({
                filter: (reaction, user) => {
                    const member = constants.mainGuild.members.cache.get(
                        user.id
                    );
                    return (
            ...
            if (message.embeds.length) {
                // Bot message
                (client.channels.cache.get(bibleVerseId) as TextChannel).send({
        ...
        if (
            constants.mainGuild.roles.cache
                .get("829658046149033985")
            ...
            if (message.content.startsWith("!responses")) {
                ...
                db.run(`INSERT INTO responses (value) VALUES (?)`, [
                    newResponse,
                ]);
                ...
                message.channel.send({ embeds: [embed] });
                db.run(
                    `DELETE FROM responses WHERE value in (SELECT value FROM responses WHERE value=(?) LIMIT 1)`,
                    [responses[index]],
                    function (err) {
                        if (err) throw err;
                    }
                ...
                try {
                    ...
                    message.channel.send(
                        "There was an error with that expression: " + e
                    );
                    return;
                }
                ...
                let responseMessage;
                try {
                    responseMessage = await message.channel.awaitMessages({
                        filter: (m) => {
                ...
                db.run(
                    `INSERT INTO triggers (expression, response) VALUES (?, ?)`,
                    [expr, text.content],
                    (err) => {
                        if (err) throw err;
                    }
                ...
                const id = parseInt(
                    message.content.substring("!remtrigger ".length)
                );
                if (isNaN(id)) {
                ...
                const text = responseMessage.get(responseMessage.firstKey());
                const match = /```(?:py(?:thon)?)?([\w\W]*)```/.exec(
                    text.content
                );
                let resultFunc: PythonActionExpression;
                const action = text.content.startsWith("```")
                    ? match
                        ? match[1]
                        : text.content
                    : text.content;
                ...
                db.run(
                    `INSERT INTO pytriggers (expression, response) VALUES (?, ?)`,
                    [expr, action],
                    (err) => {
                        if (err) throw err;
                    }
                ...
                const id = parseInt(
                    message.content.substring("!rempytrigger ".length)
                );
                if (isNaN(id)) {
                    message.channel.send("Not a number: " + id);
                    return;
        ...
        if (
            message.channel.type === "DM" &&
            message.author.id !== "694538295010656267"
        ) {
        ...
        if (message.content.startsWith("!export")) {
            if (message.member?.roles.cache.has("762592153393692682")) {
                ...
                message.channel.send({
                    content: "Export complete! Here is the data",
                    files: [attachment],
                });
            } else {
                message.reply(
            ...
            if (split.length == 1 && split[0] == "all") {
                ...
                while (
                    (messages = await (<TextChannel>channel).bulkDelete(
                        100,
                        true
                    )).size >= 100
                ) {
                ...
                const messages = await (<TextChannel>(
                    message.channel
                )).bulkDelete(num, true);
            ...
            const allMessageString = allMessages
            ...
            const allAttachments = allMessages
                .map((message) => Array.from(message.attachments.values()))
                .flat();
            const logMessage = new MessageEmbed()
                .setTitle(
                    "Bulk messages deleted in #" +
                ...
                embeds: [
                    new MessageEmbed()
                        .setAuthor({
                            name: "Clem",
                            iconURL:
                                "https://cdn.discordapp.com/avatars/708155649455816785/a803bf4737f4dec2ada1e6b0517e3b61.webp",
                        })
            ...
            message.channel.send(
                `Do Not Disturb: ${
                    ...
                    `Idle: ${
                        onlineUsers.filter((x) => x.presence.status === "idle")
                            .size
                    }`
            );
        } else {
    ...
    setInterval(() => {
        get(
            ...
            .end();
        get(
            "https://public-api.wordpress.com/rest/v1.1/sites/asiawritescreatively.wordpress.com/posts?offset=0&number=1",
            async (res) => {
            ...
            if (data.found > count) {
                ...
                const embed = new MessageEmbed()
                    ...
                    .setAuthor({
                        ...
                        iconURL: data.posts[0].author.avatar_URL,
                    })
                    .setDescription(desc)
