...
async function scheduleEventFromDoc() {
    ...
    ["Date", "Passage/Topic", "From", "To", "Host", "Bot parsed"].forEach((header, i) => {
        if(sheet.getCell(1, i).value !== header) {
            throw new Error(`Header mismatch: ${sheet.getCell(1, i).value} !== ${header}`);
        }
    });
    let y = 2;
    while(true) {
        if(sheet.getCell(y, 5).value !== null && sheet.getCell(y, 5).value !== "✓")
            throw new Error("Unexpected value in `Bot parsed` field: " + sheet.getCell(y, 5).value);
    ...
    if(!(typeof host === "string") || !(typeof passage === "string") || !(typeof startTime === "string") || !(typeof endTime === "string")) throw new Error("Invalid data in row " + y);
    const startDate = parseTimeField(startTime, date);
    const endDate = parseTimeField(endTime, date);
    if(startDate.getTime() > endDate.getTime()) throw new Error("Start time is after end time");

    console.log(`Adding event ${startDate} to ${endDate} at ${host} for ${passage}`);

    // Add event to discord
    if((await constants.mainGuild.scheduledEvents.create({
        entityType: 'EXTERNAL',
        name: "Youth Group",
...
function parseTimeField(str, baseDate) {
    ...
    const hour = match[3].startsWith("p") ? parseInt(match[1]) + 12 : parseInt(match[1]);
    const date = new Date(baseDate.getFullYear(), baseDate.getMonth(), baseDate.getDate(), hour, parseInt(match[2] ?? "0"), 0);
    return date;
}
...
function parseDateField(str) {
    ...
    if(!((typeof day === "number") && (typeof month === "number") && (typeof dayOfWeek === "number"))) throw new Error(`Date ${match[0]} is invalid: ${day}, ${month}, ${dayOfWeek}`);
    const year = new Date().getMonth() > month ? new Date().getFullYear() + 1 : new Date().getFullYear();
